...
class UploadAvatarForm(FlaskForm):
	image = FileField('Upload', validators=[FileRequired(), FileAllowed(['jpg', 'png'], 'The file format should be .jpg or .png.')])
	submit = SubmitField()
...
class CropAvatarForm(FlaskForm):
	...
	w = HiddenField()
	h = HiddenField()
	submit = SubmitField('Crop and Updata')
...
class ChangeEmailForm(FlaskForm):
	...
	def validate_email(self, field):
		if User.query.filter_by(email=field.data.lower()).first():
			raise ValidationError('The email has been exites')
