...
from albumy.forms.user import EditProfileForm, UploadAvatarForm, CropAvatarForm, ChangeEmailForm, ChangePasswordForm, NotificationSettingForm, PrivacySettingForm, DeleteAccountForm
...
def show_followers(username):
	...
	pagination = Follow.query.filter_by(followed_id=user.id).order_by(Follow.timestamp.desc()).paginate(page, per_page)
...
def upload_avatar():
	...
	if form.validate_on_submit():
		...
		db.session.commit()
		flash('Image uploaded, please crop', 'sucess')
	flash_errors(form)
...
def change_password():
	...
	if form.validate_on_submit():
		if current_user.validate_password(form.old_password.data):
			...
			flash('Old passwrod is not corrent', 'warning')
...
def change_email(token):
	if validate_token(user=current_user, token=token, operation=Operations.CHANGE_EMAIL):
		flash('Email updated', 'sucesss')
		return redirect(url_for('.index', username=current_user.username))
	else:
		flash('Inavalid or expried token', 'warning')
...
def notification_setting():
	...
	if form.validate_on_submit():
		...
		db.session.commit()
		flash('Notification settings updataed', 'success')
		return redirect(url_for('.index', username=current_user.username))
...
@user_bp.route('/settingg/account/delete', methods=['GET', 'POST'])
@login_required
def delete_account():
	...
	if form.validate_on_submit():
		...
		db.session.commit()
		flash('Your are free, bye', 'sucess')
